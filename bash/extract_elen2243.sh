...
function extractReads {
    ...
    sample=$(basename $fileHandle | cut -d_ -f1)
    bamOut=/mnt/parallel_scratch_mp2_wipe_on_august_2017/dubemar1/augerjer/MicrobiomeData/merge/bamOut
    samtools view -f 2 -h -b ${fileHandle} "gi|339443684|ref|NC_015738.1|" > YY7918/${sample}_f2_YY7918.bam
}
export -f extractReads
...
function extractReads {
    ...
    sample=$(basename $fileHandle | cut -d_ -f1)
    bamOut=/mnt/parallel_scratch_mp2_wipe_on_august_2017/dubemar1/augerjer/MicrobiomeData/merge/bamOut
    bedtools bamtobed -i YY7918/${sample}_f2_YY7918.bam -cigar | awk -F '\t' '($3-$2 > 90) { print $4 }' > YY7918/${sample}_elen2243_HitsList.txt
}
export -f extractReads
